...
class MixingTank(Tank):
    def __init__(self, capacity: float, batch: float, inlet_valve: Valve,
                 outlet_valve: Valve, level_transmitter: LevelTransmitter, mixer: Mixer,
                 opc_client: OPCClient):
        super().__init__(type(MixingTank).__name__, capacity, batch)
    ...
    def fill_and_mixing(self, time_to_mix: int):
