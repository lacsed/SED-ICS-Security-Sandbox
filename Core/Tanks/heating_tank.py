...
class HeatingTank(Tank):
    def __init__(self, capacity: float, batch: float, inlet_valve: Valve, temperature_transmitter: 
                 TemperatureTransmitter, level_transmitter: LevelTransmitter, opc_client: OPCClient):
        super().__init__(type(HeatingTank).__name__, capacity, batch)
    ...
    def heat_tank(self, initial_temperature: float, final_temperature: float, time_to_heat: int):
        ...
        self.temperature_transmitter.set_current_temperature(initial_temperature)
        ...
        print(f"Tank '{self.name}' heated to {self.temperature_transmitter.temperature}°C.")

    def fill_and_heating(self, initial_temperature: float, final_temperature: float, time_to_heat: int):
        ...
        self.heat_tank(initial_temperature, final_temperature, time_to_heat)
